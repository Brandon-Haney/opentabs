...
### Self-Healing Session Token (rejected request returns its own replacement)
Enterprise apps that pair an HttpOnly session cookie with a rotating anti-CSRF session token (e.g. ServiceNow's `g_ck` sent as `X-UserToken`) frequently hand back a **fresh token on the rejected response itself**, in a header such as `X-UserToken-Response`. This makes the token self-healing: read the current token from the page global, send it on every request, and when a request returns 401, take the replacement from the response header, persist it with `setAuthCache`, and retry once. No page reload and no re-login is needed.

This matters because the page global holding the token is only correct at load time. A tab left open for hours holds a token the server has long since rotated away from, so *every* tool call fails until the token is refreshed — a routine situation, not an edge case, since users keep these tabs open all day. Prefer the cached token over the page global once a request has been made, because after a heal the cache is fresher than the global.

The retry cannot be built on `fetchJSON`/`fetchFromPage`: those throw on a non-ok response and discard the `Response` before the caller can read the replacement header. Compose a small helper that calls `fetch` directly for the send, inspects `response.status === 401` and the refresh header, and then maps any surviving failure through `httpStatusToToolError` so error classification still comes from the SDK. Always send the token header even when the cached token is known to be stale — see gotcha #50 for why omitting it is never a safe fallback.
...
50. **A 401 carrying `WWW-Authenticate: Basic` opens a blocking native browser sign-in dialog.** When an adapter `fetch()` receives 401 with a non-empty `WWW-Authenticate` header, Chrome renders its own username/password prompt over the tab. That dialog is tab-modal: it blocks every subsequent extension command, so the plugin — and every other tool operating on that tab — stops responding until a human dismisses it. This is far worse than an ordinary error, and it can be triggered by a single exploratory `fetch` during Phase 2. Enterprise REST APIs that accept both session and API-client auth commonly behave this way: a request carrying the app's own session header is answered with a plain 401 (often with an *empty* `WWW-Authenticate`, deliberately suppressing the prompt), while a request with no auth header at all is treated as an API client and challenged. The rule: once you know which header the app's own UI sends, send it on **every** request including retries and health checks, and never let "try it without auth to see what happens" be a fallback path. During Phase 2, if a probe raises a browser sign-in box, that is the signal — dismiss it, find the header the UI sends, and never omit it again.
51. **An endpoint can return 200 with a well-formed envelope that contains no records.** Some search endpoints return the *catalog of what is searchable* — groups, table names, labels, filters — rather than results, and the response is structurally indistinguishable from a result set at a glance (`{result:{groups:[{tables:[{name,label,...}]}]}}`). Skimming it during Phase 2 and seeing plausible nesting is not verification. Before building a tool on any search or list endpoint, confirm the response contains **actual record identifiers** for a term you know matches, and confirm the count changes when the term changes. This is the read-side counterpart of gotcha #34. When a search endpoint turns out to be a catalog, look for a full-text operator on the app's ordinary record API instead — many platforms expose their search index as a query operator rather than a separate endpoint, which is both simpler and more composable than the dedicated search route.
52. **Metadata tables are often withheld by access rules while the same metadata is readable through a UI endpoint.** Enterprise apps commonly restrict the tables that describe the schema (choice lists, field definitions, dictionary entries) even for users who can freely read the records those definitions describe — querying them returns 403 "insufficient rights" rather than an empty set. The app's own forms still render dropdowns, so the information is reachable: look for a UI-oriented metadata endpoint (e.g., `/api/.../ui/meta/<table>`) that returns column definitions and their allowed values in one call. Prefer that endpoint over the raw metadata table for any `describe_*` or `list_*_choices` tool, and treat a 403 on a metadata table as a routing problem, not a dead end.

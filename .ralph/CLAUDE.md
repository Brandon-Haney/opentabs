...
- **Dev tooling must ignore worktrees.** Biome and knip will scan `.ralph/worktrees/` and `.claude/worktrees/` unless explicitly excluded. These exclusions are in `biome.json` and `knip.ts`. Forgetting this causes lint crashes (no tsconfig for worktree files) and knip reporting hundreds of false "unused files."

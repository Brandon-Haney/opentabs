...
`ToolError` fixes this by adding structured metadata to every error:
...
- **`details`** — flat, primitive-valued triage data such as the upstream HTTP status or request id (optional; never a full URL)
...
One thing I wanted to get right: you throw a `ToolError` in your tool handler, and its code, category, and retry hints arrive at the AI agent intact — no special plumbing on your end. Here's the path it takes:

1. **Tool handler** throws `ToolError` in the page context
2. **Browser extension** serializes the error, preserving `code`, `category`, `retryable`, `retryAfterMs`, and `details`
3. **MCP server** formats the error into a human-readable prefix line and a machine-readable JSON block, and records it — `details` included — in the audit log
...
`details` — the flat map you attach with `withDetails()` or that `httpStatusToToolError` fills with `httpStatus` — is recorded in the audit log (`opentabs audit --json`) so an upstream failure can be traced to its status and request id later; the agent-facing JSON block carries `code`, `category`, `retryable`, and `retryAfterMs`. Keep detail values to hosts, identifiers, and numbers: never a full URL.

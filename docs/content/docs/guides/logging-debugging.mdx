...
```bash
Time       Tool                      Origin                   OK   Duration
10:15:30   slack__list_channels       https://app.slack.com    ✓    120ms
10:15:45   slack__send_message        https://app.slack.com    ✗    30.0s
10:16:02   slack__list_channels       https://app.slack.com    ✓    95ms
```

A red `✗` means the tool call failed. The `Origin` column is the scheme and host of the tab the tool targeted (never the full URL); it is blank when no tab was resolved. Filter by plugin to focus on what matters:
...
The `--json` output is where the real detail lives — the targeted tab (`tabId` plus `tabOrigin`), and for failures the error code, category, message, and structured `details` that tell you exactly what went wrong:
...
`error.details` holds the remaining primitive fields of the plugin's error — retry metadata, the upstream HTTP status, a request id — with string values sanitized. It is omitted when the plugin supplied nothing beyond the code and message, or when the serialized object would exceed 4 KB.

...
- **`name`** — unique identifier used as the tool prefix in MCP. A tool called `get_items` becomes `my-app__get_items` when exposed to AI agents.
...
- **`name`** — `snake_case`, descriptive of the action. Becomes `<plugin>__<name>` in MCP (e.g., `my-app__get_items`).
...
`httpStatusToToolError` maps HTTP status codes to appropriate error categories: 401/403 → `auth` (not retryable), 404 → `notFound` (not retryable), 429 → `rateLimited` (retryable, parses `Retry-After` header), 400/422 → `validation` (not retryable), 408 → `timeout` (retryable), 503 → `internal` (retryable, parses `Retry-After` header), 500/502/504 → `internal` (retryable), other 5xx such as 501/505 → `internal` (not retryable), other 4xx → no category (not retryable). Every error it returns carries `details.httpStatus` for the audit log.

If the request itself is flaky — a proxy returning 500s or `Failed to fetch` on an otherwise healthy API — wrap it in `fetchWithRetry` instead of hand-rolling a loop. It retries transient statuses and network errors for GET/HEAD/OPTIONS with jittered backoff and returns the final `Response` for you to classify:

```typescript
import { fetchWithRetry, httpStatusToToolError } from '@opentabs-dev/plugin-sdk';

const response = await fetchWithRetry('/api/items', { credentials: 'include' });
if (!response.ok) {
  throw httpStatusToToolError(response, `Failed to fetch items: ${response.status}`);
}
```
...
4. Ask your AI agent to use a tool: *"Use the my-app__get_items tool to list items"*

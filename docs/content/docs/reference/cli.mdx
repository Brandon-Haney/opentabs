...
Running `opentabs config set tool.` (with a trailing dot and no value) lists all available tool names from the running server. Running `opentabs config set browser-tool.` (with a trailing dot and no value) lists all available browser tool names from the running server.

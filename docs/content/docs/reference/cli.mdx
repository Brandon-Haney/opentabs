...
<Table>
  ...
  <Table.Body>
    ...
    <Table.Row>
      ...
      <Table.Cell>—</Table.Cell>
      <Table.Cell>Filter by tool name (e.g., `slack__send_message`).</Table.Cell>
    </Table.Row>
    <Table.Row>
...
Each entry shows the timestamp, tool name, tab origin (scheme and host of the tab the tool targeted, when known), success/failure status, and duration. With `--json`, entries also carry the numeric `tabId`, and failed entries include the error code, category, message, and any structured `details` (for example `retryable`, `retryAfterMs`, and the upstream HTTP status).
...
# Combine filters
opentabs audit --tool slack__send_message --since 30m
...
<Table>
  ...
  <Table.Body>
    <Table.Row>
      <Table.Cell className="font-medium">`<name>`</Table.Cell>
      <Table.Cell>Yes</Table.Cell>
      <Table.Cell>Tool name (e.g., `slack__send_message`, `browser_list_tabs`).</Table.Cell>
    </Table.Row>
  </Table.Body>
</Table>
...
opentabs tool call slack__send_message --params '{"channel":"C123","text":"hello"}'

# Target a specific tab
opentabs tool call slack__send_message '{"channel":"C123","text":"hi"}' --tab-id 42
```

...
export const buildAddSlideBody = (
  ...
  // The root, copied verbatim with only the slide list changed.
  const newRootProperties: (string | number)[] = [];
  for (let i = 0; i + 1 < ctx.rootProperties.length; i += 2) {
    const key = ctx.rootProperties[i];
    const value = ctx.rootProperties[i + 1];
    if (key === undefined || value === undefined) continue;
    newRootProperties.push(key, key === PROP_SLIDE_LIST ? newSlideList : value);
  }

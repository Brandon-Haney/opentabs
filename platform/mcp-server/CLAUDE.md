...
Discovers plugins, registers their tools as MCP capabilities, dispatches tool calls to the Chrome extension via WebSocket, receives plugin log entries and forwards them to MCP clients via the logging capability, converts tool progress notifications into MCP `notifications/progress` events, and serves health/config endpoints. The server maintains an in-memory audit log of the last 500 tool invocations, queryable via `GET /audit` (with Bearer auth), with aggregate stats included in the `/health` response's `auditSummary` field. Each entry (`AuditEntry` from `@opentabs-dev/shared`, also persisted as NDJSON to `~/.opentabs/audit.log`) records the tool, plugin, success, duration, the dispatch-target tab (`tabId` — the extension-echoed tab when the envelope carries one, else the caller-targeted or instance-resolved tab — plus `tabOrigin`, scheme + host only, resolved from the cached tab mapping before the tool runs so a navigating tool records the origin it was dispatched against) and, on failure, `error.{code,message,category,details}` where `details` is the sanitized, size-bounded (4 KB) primitive remainder of the extension's structured error data (`retryable`, `retryAfterMs`, upstream status, request id). The extension's `tabId` echo is stripped from the tool output before it reaches the MCP client.

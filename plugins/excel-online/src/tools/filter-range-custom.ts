 ...
 * `equals` (0), `contains` (6), `greater_than` (8), `greater_or_equal` (9),
 * `less_than` (10), and `between` (12) are capture-verified; the remaining
 * codes are each +1 of a verified code (the +1 rule confirmed live at 9).
 * `between`/`not_between` require both `value` and `value2`.
 */
const OPERATORS = {
  equals: 0,
  not_equal: 1,
...
export const filterRangeCustom = defineTool({
  ...
  description:
    ...
    'less_than, less_or_equal, contains, does_not_contain (all take one "value"), and between, not_between (take ' +
    '"value" as the lower bound and "value2" as the upper). "column" is the zero-based column index within the ' +
    "range. Not available through the standard workbook API — driven through Excel's internal service via the frame bridge.",

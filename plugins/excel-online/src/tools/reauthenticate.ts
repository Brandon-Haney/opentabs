...
import { z } from 'zod';
import { isSharePointWorkbook } from '../excel-api.js';

/** localStorage key the pre-script mirrors the captured Graph token to. */
const LS_TOKEN_KEY = '__opentabs_excel_graph_token';

/**
 * Recover from a stale Graph token on SharePoint/OneDrive-hosted workbooks.
 *
 ...
 *
 * Caller protocol: when `excel-online__*` tools return an `AUTH_ERROR` whose
 * message ends with "Call `excel-online__reauthenticate` to recover", invoke
...
export const reauthenticate = defineTool({
  ...
  description:
    ...
    'Use this when another Excel tool returns an authentication error pointing at this tool. ' +
